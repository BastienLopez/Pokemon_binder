...
class TCGdxService {
  ...
  static async getSeries() {
    try {
      const response = await fetch(`${TCGDX_BASE_URL}/series`);
      if (!response.ok) {
  ...
  static async getSetsBySerie(serieId) {
    ...
    try {
      const response = await fetch(`${TCGDX_BASE_URL}/sets?serie=${serieId}`);
      if (!response.ok) {
        throw new Error(`Erreur HTTP: ${response.status}`);
      }
      return await response.json();
  ...
  static async getSets() {
    try {
      const response = await fetch(`${TCGDX_BASE_URL}/sets`);
      if (!response.ok) {
        throw new Error(`Erreur HTTP: ${response.status}`);
      }
      return await response.json();
  ...
  static async getCardsBySet(setId) {
    ...
    try {
      const response = await fetch(`${TCGDX_BASE_URL}/cards?set.id=${setId}`);
      if (!response.ok) {
        throw new Error(`Erreur HTTP: ${response.status}`);
      }
      return await response.json();
  ...
  static async getCard(cardId) {
    ...
    try {
      const response = await fetch(`${TCGDX_BASE_URL}/cards/${cardId}`);
      if (!response.ok) {
        throw new Error(`Erreur HTTP: ${response.status}`);
      }
      return await response.json();
  ...
  /**
   * Formate l'URL de l'image haute qualité d'une carte avec fallback multi-API
   * @param {Object} card - Objet carte contenant le champ image ou id
  ...
  static getHighQualityImageUrl(card) {
    ...
    if (card?.id) {
      ...
      // Format correct pour TCGdx: https://assets.tcgdx.net/fr/serie/set/numero/high.webp
  ...
  static getNextFallbackUrl(card, attemptIndex = 0) {
    ...
    return fallbackUrls[attemptIndex] || PLACEHOLDER_IMAGE_SVG;
  }
